...
def haversine_distance(a, b):
    ...
    if torch.is_tensor(a.lat) and torch.is_tensor(a.lng) and torch.is_tensor(b.lat) and torch.is_tensor(b.lng):
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        delta_lat = torch.deg2rad(torch.sub(b.lat, a.lat))
        delta_lng = torch.deg2rad(torch.sub(b.lng, a.lng))
        v0 = torch.mul(torch.sin(torch.div(delta_lat, 2.0)), torch.sin(torch.div(delta_lat, 2.0)))
        v1 = torch.mul(torch.sin(torch.div(delta_lng, 2.0)), torch.sin(torch.div(delta_lng, 2.0)))
        v2 = torch.mul(torch.cos(torch.deg2rad(a.lat)), torch.cos(torch.deg2rad(b.lat)))
        h = torch.add(v0, torch.mul(v1, v2))
        c = torch.mul(torch.atan2(torch.sqrt(h), torch.sqrt(torch.sub(torch.tensor([1.], device=device), h))), 2.0)
        d = torch.mul(c, EARTH_MEAN_RADIUS_METER)
