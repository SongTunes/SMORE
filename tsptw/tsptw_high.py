...
parser.add_argument('--epoch', default=1500, help="number of epochs")
...
load_root = './model/load-root'
load_root_high = './model/load-root-high'

print('=========================')
print('prepare to train high model')
print('=========================')
...
# state_high = torch.load(load_root_high)
...
for epoch in range(n_epoch):
    ...
    for i in tqdm(range(steps)):
        ...
        max_grad_norm = 1.0
        torch.nn.utils.clip_grad_norm_(model_high.parameters(),
        ...
        if i % 50 == 0:
            ...
            gap = (total_time_cost.mean().item() - solutions.mean()) / solutions.mean()
            gap_epoch += gap
            print('validation result:{}, optimal result:{}, accuracy:{}, gap:{}'
                  .format(total_time_cost.mean().item(), solutions.mean(), accuracy, gap))

            val_mean.append(total_time_cost.mean().item())
            val_std.append(total_time_cost.std().item())
            val_accuracy.append(accuracy)
    print('epoch:{}, gap sum:{}, best:{}'.format(epoch, gap_epoch, gap_best))
    if gap_epoch <= gap_best:
        gap_best = gap_epoch
        state_best = {
            'epoch': epoch,
            'model': model_high.state_dict(),
            'optimizer': optimizer.state_dict()
        }
        torch.save(state_best, save_root + '_' + str(epoch) + '_best_high.pt')
        print('save best in epoch {}'.format(epoch))
    state = {
        'epoch': epoch,
        'model': model_high.state_dict(),
        'optimizer': optimizer.state_dict()
    }
    torch.save(state, save_root + '_vien_high.pt')
